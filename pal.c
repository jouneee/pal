...
void generate_scheme(uint8_t *pixels, int w, int h, Config config, Color *out_palette, Color *out_bg, Color *out_fg) {
    ...
    for (int y = 0; y < h && count < 1024; y += step_y) {
        for (int x = 0; x < w && count < 1024; x += step_x) {
    ...
    *out_fg = lightest;
    ...
    for (int iter = 0; iter < 10; iter++) {
        ...
        for (int s = 0; s < 1024; s++) {
            ...
            for (int k = 0; k < 16; k++) {
                float d = color_dist(samples[s], centers[k]);
                if (d < best_dist) {
                    best_dist = d;
                    best_idx = k;
                }
            ...
            r_sum[best_idx] += samples[s].r;
            g_sum[best_idx] += samples[s].g; 
            b_sum[best_idx] += samples[s].b;
            counts[best_idx]++;
        }
...
uint32_t final_hash(uint8_t *pixels, size_t len, Config config) 
{
    ...
    h ^= fnv32_hash((unsigned char *)&config.function,      sizeof(config.num_accents));
    return h;
}
...
int main(int argc, char **argv) {
    ...
    Config config = { .bgfg = 1, .num_accents = 16, .saturation = 1.0, .function = 1 };

    if (argc <= 0) {
        printf("Usage: %s [-n][-f][-p][-s] <image1> <image2> ...\n", program);
        printf("\n");
        printf("\t-n \tDont print background and foreground\n");
        printf("\t-p \tAmount of accent colors\n");
        printf("\t-s \tSaturation (float)\n");
        printf("\t-f \tColor picking function (0 - Area Average, 1 - K-Means)\n");
        return 1;
    }
    ...
    while (argc > 0) {
        ...
        if (strcmp(flag,"-n") == 0) {
            ...
            config.function = atoi(args_shift(&argc, &argv));
            if (config.function > 1 || config.function < 0) {
                return 1;
            }
        }
        else {
